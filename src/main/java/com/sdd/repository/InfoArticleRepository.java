...
public interface InfoArticleRepository extends JpaRepository<InfoArticle,Integer>{
    ...
    @Query(value = "select info from  InfoArticle info where  (?1 is null or info.name = concat('%',concat(?1,'%') ))  " +
            " and (?2 is null or info.infoId = ?2) and (?3 is null or info.typeId = ?3) and (?4 is null or info.twoTypeId = ?4)")
    Page<InfoArticle> page(String name, Integer infoId,Integer typeId,Integer twoTypeId,Pageable p);
}
